...
import {
    ...
    ChevronRight, CreditCard, Maximize, ShieldCheck, Building
} from 'lucide-react';
...
const ManageEvent = () => {
    ...
    const fetchEvents = async () => {
        setLoading(true);
        try {
            // Staff access a global endpoint, Employers access their specific dashboard
            const endpoint = isStaff 
                ? `/admin/dashboard/events?status=${filter}` 
                : `/employer/dashboard/events?status=${filter}`;
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 className="text-3xl font-black italic text-gray-900 tracking-tight">
                        {isStaff ? 'University' : 'Recruitment'} <span className="text-[#A10022]">Events</span>
                    </h1>
                    <p className="text-gray-500 font-medium">
                        {isStaff 
                            ? "Oversee all campus activities and student engagement sessions." 
                            : "Build your brand on campus through workshops and sessions."}
                    </p>
                </div>
                <button
                ...
                >
                    <Plus size={18} /> {isStaff ? 'Create Global Event' : 'Host New Event'}
                </button>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-[2.5rem] border border-gray-100 shadow-sm">
                ...
                <div className="relative w-full md:w-80">
                    ...
                    <input
                        type="text"
                        placeholder={isStaff ? "Search by title or organizer..." : "Search your events..."}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {filteredEvents.map(event => (
                        <EventCard 
                            ...
                            event={event} 
                            isStaff={isStaff}
                            onOpenScanner={() => setScanningEvent(event)} 
                        />
                    ))}
                </div>
            ) : (
            ...
            {isModalOpen && (
                <CreateEventModal 
                    ...
                    isStaff={isStaff} // Pass role to modal for conditional fields
                />
            )}
...
const EventCard = ({ event, onOpenScanner, isStaff }) => {
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex flex-col items-end gap-2">
                    ...
                    {/* Staff View: Show who organized this */}
                    {isStaff && (
                        <span className="flex items-center gap-1 text-[7px] font-black text-gray-400 uppercase tracking-tighter">
                            <Building size={10} /> {event.organizerName || "University Dept"}
                        </span>
                    )}
                </div>
            </div>
