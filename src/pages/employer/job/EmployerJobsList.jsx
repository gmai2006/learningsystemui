...
const EmployerJobsList = () => {
    ...
    const toggleJobStatus = async (jobId, currentStatus) => {
        try {
            await apiClient.put(`/employer/dashboard/jobs/${jobId}/status`, !currentStatus);
            fetchJobs(); 
        } catch (err) {
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
                <div className="relative flex-1 max-w-md">
                    <Tooltip text="Search by job title">
                ...
                <Tooltip text="Create a new job listing">
                    <button
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {filteredJobs.map((job) => (
                            <tr key={job.id} className="hover:bg-gray-50/50 transition-colors group">
                                <td className="p-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <Tooltip text="Job Classification">
                                                <span className="text-[9px] font-black text-[#A10022] bg-red-50 px-2 py-0.5 rounded-md uppercase tracking-tighter cursor-help">
                                                    {job.category}
                                                </span>
                                            </Tooltip>
                                        </div>
                                ...
                                <td className="p-6">
                                    <Tooltip text="Total candidate submissions">
                                ...
                                <td className="p-6">
                                    <Tooltip text={job.isActive ? "Visible to students" : "Archived"}>
                                        <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest cursor-help ${job.isActive ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-400'}`}>
